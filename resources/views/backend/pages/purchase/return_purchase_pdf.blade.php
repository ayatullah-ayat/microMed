        ...
        <table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; " cellpadding="8">
            ...
            <tbody>
                ...
                @endphp
                    ...
                    @foreach ( $returnpurchases as $item )
                    @php
                        $all_total_qty          += $item->returned_qty;
                        $all_total_amount       += $item->subtotal;
                    @endphp
                        <tr style="border-bottom: 1px solid #000;">
                            <td align="left">{{  $loop->iteration }}</td>
                            <td align="left">{{  $item->invoice_no ?? 'N/A' }}</td>
                            <td align="left">{{  $item->purchase->supplier->supplier_name ?? 'N/A' }}</td>
                            <td align="left">{{  $item->created_at ? date('Y-m-d', strtotime($item->created_at)) : 'N/A' }}</td>
                            <td align="right">{{ $item->returned_qty ?? '0.0' }}</td>
                            <td align="right">{{ $item->subtotal ?? '0.0' }}</td>
                        </tr>
                    @endforeach
                @endisset
                <!-- END ITEMS HERE -->
                <tr>
                    ...
                    <td class="totals cost">{{ $all_total_amount ?? '0.0' }}</td>
                </tr>
            </tbody>
        </table>
